...
export default function InfiniteMarquee() {
  ...
  return (
    <section className="relative py-12 bg-gradient-to-b from-[#0c0908] via-[#140e0a] to-[#0c0908] overflow-hidden border-y border-[#d4a359]/15">
      {/* Edge-fade masks for seamless infinite feel */}
      <div className="absolute top-0 left-0 bottom-0 w-24 sm:w-48 z-10 pointer-events-none bg-gradient-to-r from-[#0c0908] to-transparent" />
      <div className="absolute top-0 right-0 bottom-0 w-24 sm:w-48 z-10 pointer-events-none bg-gradient-to-l from-[#0c0908] to-transparent" />
        ...
        <div className="animate-marquee-left flex items-center gap-6 whitespace-nowrap">
          {topRowItems.concat(topRowItems).map((item, idx) => {
            ...
            return (
              <div
                key={`top-${idx}`}
                className="inline-flex items-center gap-2.5 px-5 py-2.5 rounded-full border border-[#d4a359]/25 bg-[#1f140f]/75 backdrop-blur-md text-sm font-medium text-[#faf6f0] hover:border-[#d4a359]/70 hover:bg-[#2a1a13] transition-colors shadow-sm"
              >
        ...
        <div className="animate-marquee-right flex items-center gap-6 whitespace-nowrap">
          {bottomRowItems.concat(bottomRowItems).map((item, idx) => {
            ...
            return (
              <div
                key={`bottom-${idx}`}
                className="inline-flex items-center gap-2.5 px-5 py-2.5 rounded-full border border-white/10 bg-[#160f0b]/80 backdrop-blur-md text-sm font-medium text-[#ebdccb] hover:border-[#d4a359]/50 hover:bg-[#221610] transition-colors shadow-sm"
              >
